...
static void putString(char **, cput_t, char *, int, bool);
...
static void putString(char **s, cput_t put, char *out, int pad, bool ldng_zero) {
    char c = 0;
...
static size_t kvasprintf(char **dest, cput_t put, char *fmt, va_list arg) {
    ...
    while ((c = *fmt++)) {
        ...
        length += count;
        if (str_buf) {
            putString(dest, put, str_buf, pad, ldng_zero);
            str_buf = NULL;
        } else {
            putString(dest, put, num_buf, pad, ldng_zero);
        }
    }
vkprint_end:
    return length;
...
/* calls kPutChar to put a char on the screen */
