...
global g_start
g_start:
    mov ecx, (boot_pageDirectory - KERNEL_VIRTUAL_BASE)
...
start_higher_half:
    ...
    mov dword [boot_pageDirectory], 0
    invlpg [0]

    mov esp, kernel_stackTop ; set up stack pointer
...
extern baseIrqHandler
extern g_need_reschedule
extern schedule
; calls baseIrqHandler defined in 'irq.c'
irq_common_stub:
    pushregs
...
; when switchToThread is called, the stack looks like:
;       - pointer to thread
;       - func return address
...
switchToThread:
    ...
    mov [eax+4], dword 0        ; clear numTicks field
...
global startUserMode
startUserMode:
    cli                         ; disable interrupts

    ;xchg bx, bx                ; Bochs magic breakpoint

    mov eax, [esp]              ; load addr of startFunc from stack
    ...
    sub esp, 4                  ; pretend we pushed the return address (startFunc)
                                ; startFunc is already here!
    iret                        ; 'return' to user mode thread code


section .bss
global kernelStackBottom
global kernel_stackTop
global mainThreadAddr
mainThreadAddr:
